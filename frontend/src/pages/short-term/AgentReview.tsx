...
import { AgentChat } from "@/components/short-term/AgentChat";
import {
  ConsecPremiumCard, CycleCard, LadderCard, MoneyEffectCard, PromotionCard, splitMetrics,
} from "@/components/short-term/EmotionMetricsPanel";
import {
  BoardSystemSection, DiffView, Ledger, LossEffectSection, Matrix, SealQualitySection,
  StatsView, Themes, ThemeTreeView,
} from "@/components/short-term/MarketFactsPanel";
import { BreadthPanel } from "@/components/short-term/BreadthPanel";
import { TrendPanel } from "@/components/short-term/TrendPanel";
import {
...
export function AgentReview() {
  ...
  async function generate() {
    ...
    try {
      ...
      stopPolling();
      if (alive.current) {
        setErr("连接不到后端。请在项目根目录运行 .\\start.ps1，或手动启动 backend（端口 8900）");
      }
      return;
    }
